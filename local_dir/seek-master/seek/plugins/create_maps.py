...
def _fill_maps(maps, map_counts, paths, pixels, mapper, ctx):
    ...
    with RestructuredTODStore(paths) as store:
        for i, tod_pixel in enumerate(pixels):
            ...
            values, counts = mapper.get_mapped_values(re_data, ctx)     #values = mediana do valor do pixel | counts = contagem de pixels nao mascarados.
            maps[:, xy_ind, i] = values                                 # aqui colocamos o valor no pixel. Precisamos colocar valor nos pixels no entorno tambem
