...
class Plugin(BasePlugin):
    ...
    def __call__(self):
        if self.ctx.tod_vx.shape[0] > 0:
            ...
            azs = np.radians(np.interp(time_axis, coords['Time'], coords['AzAntenna']))     #interpolando sobre os instantes do TOD de acordo com os dados do .txt
            ...
            self.ctx.coords = Coords(strategy[:, 0], strategy[:, 1], azs, els, time_axis)       #local onde eh definido RA e DEC
        else:
