...
class Plugin(BasePlugin):
    ...
    def __call__(self):
        ...
        pixels = np.floor(np.sqrt(beam_area / pixel_area))      # numero de pixels que o feixe ocupa.
        pixels = pixels if pixels%2==1 else pixels+1
        
        pixel_size = hp.max_pixrad(params.beam_nside)           # maior distancia entre o centro do pixel e sua borda (diagonal/2)
        theta = (np.linspace(0, pixels, pixels*2+1)-pixels/2)*(pixel_size)      #existe jeito mais inteligente de se fazer isso.
        phi = theta
